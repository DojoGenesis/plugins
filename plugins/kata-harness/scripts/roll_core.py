...
USAGE (all verbs accept --root <dir> and --now YYYY-MM-DD; timer verbs also --now-ts <ISO8601>):
  python3 roll_core.py roll-start --reps 3                      # or --minutes 25 --cadence 25m --timer
  python3 roll_core.py roll-tick                                # surface + stage exactly ONE bring
  python3 roll_core.py roll-resolve --kind sent --note "..."    # human executed -> log the outcome
  python3 roll_core.py roll-skip --note "<reason>"              # conscious decline (reason required)
  python3 roll_core.py roll-elapse                              # window passed; bring stays open
  python3 roll_core.py roll-pause | roll-resume | roll-abort
  python3 roll_core.py roll-status                              # regenerate + print the roll face
  python3 roll_core.py roll-check                               # conformance + parity gate (exit 1 on drift)
"""
...
def roll_status_text(actions, entries, events, today, now_ts=None):
    ...
    L.append(f"_Generated by roll_core.py for {today.isoformat()} — regenerate: "
             "`python3 roll_core.py roll-status` · gate: `python3 roll_core.py roll-check`_")
    L.append("")
    ...
    if session is None:
        L.append("**No roll yet.** Start one: `python3 roll_core.py roll-start --reps N` "
                 "(or `--minutes T`).")
        L.append("")
        return "\n".join(L) + "\n"
