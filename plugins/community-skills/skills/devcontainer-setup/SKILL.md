...
description: Creates devcontainers with Claude Code, language-specific tooling (Python/Node/Rust/Go), and persistent volumes. Use when adding devcontainer support to a project, setting up isolated development environments, or configuring sandboxed Claude Code workspaces. Trigger phrases: "creates devcontainers with claude code, language-specific tooling (python/node/rust/go), and persistent volumes".
