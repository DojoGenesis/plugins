...
description: "Never poll background agents with sleep loops; continue working and check status files only when needed. Use when tempted to loop-wait on agents. Trigger phrases: \"stop polling agents\", \"sleep loop waste\", \"agent completion check\"."
