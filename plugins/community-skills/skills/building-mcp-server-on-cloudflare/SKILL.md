...
description: "Builds remote MCP (Model Context Protocol) servers on Cloudflare Workers with tool definitions, OAuth authentication, and production deployment. Use when: 'build MCP server on Cloudflare', 'remote MCP', 'deploy MCP', 'MCP OAuth', 'MCP authentication', 'MCP deployment', 'create MCP tools on Workers'."
---
